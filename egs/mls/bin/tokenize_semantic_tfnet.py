...
parser.add_argument("--output_dir", type=str, default="/scratch/data/Libritts/tokenized_sem_tfnet_16k", help="Output log file path") 
config = {}
...
for cut_ in tqdm(cut_set):
    print(cut_)
    quit()
    wav_file_path = cut_.recording.sources[0].source
    wav_file_path_1 = wav_file_path.split('LibriTTS')[1][1:]
    audio_file =os.path.join(wav_dir, wav_file_path_1)
    try:
        frame_size = 320
        shift = 160
        ...
        inds = inds.tolist()
        cut_.supervisions[0].custom['tokens']['semantic_tokens_tfnet']=inds[0]
        # print(inds)
    except Exception:
            traceback.print_exc()
