...
def main():
    ...
    if args.input_language==1: # chinese
    ...
    elif args.input_language==0: # english
        ckpt_path = "/home/v-zhijunjia/zhijundata_small_v2/data_local/data/valle-tensorboard-models/other_models/hubert/hubert_base_ls960.pt"
        layer = 9
        km_path = "/home/v-zhijunjia/zhijundata_small_v2/data_local/data/valle-tensorboard-models/other_models/hubert/hubert_base_ls960_L9_km500.bin"
    ...
    with get_executor() as ex:
        for partition, m in manifests.items(): 
            ...
            try:
                # cut_set = CutSet.from_file(f'/raid/dataset/lhotse_dataset/mls/cut_lhotse/mls-english_cuts_train_{i}.jsonl.gz')
            ...
            if args.audio_extractor:
                if args.audio_extractor == "Encodec":
                    ...
                    storage_path = (
                        f"{args.output_dir}/{args.prefix}_tfcodec_{partition}"
                    )
                ...
                cut_set = cut_set.resample(args.acoustic_sample)
                    ...
                    # #     )
                # abstrict acoustic tokens . acoustic tokens kept in .h5 file
                # with torch.no_grad():
                #     if (
            ...
            if args.prefix == "l1_l2_arctic":
                ...
                with open("/home/v-zhijunjia/zhijundata_small_v2/data/l1_l2_arctic/data/Accents/combine_L1_L2/semantic_tokens_dict/hubert_native_more_arctic_semantic_dic.json", "r") as json_file_1:  
                    semantic_loaded_dict = json.load(json_file_1)              
                with open("/home/v-zhijunjia/zhijundata_small_v2/data/l1_l2_arctic/data/Accents/combine_L1_L2/semantic_tokens_dict/hubert_indian_l1_l2_arctic_semantic_dic.json", "r") as json_file_1:  
                    indian_semantic_loaded_dict = json.load(json_file_1)          
            if args.text_extractor:
                if (
                    args.prefix == "baker"
                    ...
                    for c in tqdm(cut_set):
                        ...
                        if args.input_language==1: # english abstrict semantic tokens
                        ...
                        if args.prefix == "ljspeech":
                            ...
                            try:
                                ...
                                text_super = c.supervisions[0].text
                                print(f"error text:{text_super}")
                                text_super_re = re.sub(r'\s+', ' ', text_super)  
                                accent = c.supervisions[0].custom["accent"]
                                phonemes = tokenize_text(
                                    text_tokenizer, text=text_super_re
                                )

                            c.supervisions[0].custom = {'accent':accent}    
                            ...
                            try:
                                ...
                                native_semantic_tokens= None
                                print(e)

                            try:
                                indian_semantic_tokens=indian_semantic_loaded_dict[file_name]
                            except Exception as e:
                                indian_semantic_tokens= None
                                print(e)
                            ...
                            # assert indian_semantic_tokens, "Error: indian_semantic_tokens list is empty"
                            accent = c.supervisions[0].custom["accent"]
                            ...
                            c.supervisions[0].custom = {'accent':accent, 'native_semantic_tokens': native_semantic_tokens, 'indian_semantic_tokens':indian_semantic_tokens}  
                        ...
                        if args.add_semantic is True:
                            ...
                            print(print("not here"))
                            c.supervisions[0].custom["tokens"] = {"text": phonemes}

                        # print(f"text: {phonemes}, lab: {lab}")
                        unique_symbols.update(phonemes)
