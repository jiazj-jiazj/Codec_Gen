...
def parser():
    ...
    args = easydict.EasyDict({
        ...
        "region": "westus2",
        "cpu": False,
        "exp_name": "zhijun_pytorch_v2", #PLC_pytorch/PLC_synthesize/PLC_challenge/PLC_multi_task
        "info": "timedelay_demo_usegate_true",
        "nb_repro": 1,
    ...
    for i, run in enumerate(last_runs):   
        ...
        for log_file in details_with_logs['logFiles']:  
            ...
            if log_file.endswith('70_driver_log.txt'):  
                ...
                if exp_dir_match:  
                    bef_exp_dir_value = exp_dir_match.group(1)  
                    exp_dir_value = bef_exp_dir_value.split('/')[-1]
                    if exp_dir_value =="":
