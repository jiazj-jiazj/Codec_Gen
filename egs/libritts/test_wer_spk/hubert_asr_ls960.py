        ...
        for file_name in files:  
            ...
            if file_path.endswith(".wav") or file_path.endswith(".flac"):  
                ...
                with torch.no_grad():  
                    ...
                    if is_gt is True:
                        txt_file_name = txt_file_name.replace("gt", "asr")
                    output_file_path = os.path.join(root, txt_file_name)  

                    # 将transcript写入到新的txt文件中  
                    with open(output_file_path, "w") as output_file:  
                        output_file.write(transcription) 

def asr_vc_benchmark_dir(input_dir, is_gt):
    processor = Wav2Vec2Processor.from_pretrained("facebook/hubert-large-ls960-ft")
    model = HubertForCTC.from_pretrained("facebook/hubert-large-ls960-ft")
    # 检查GPU是否可用，并将模型移动到GPU  
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  
