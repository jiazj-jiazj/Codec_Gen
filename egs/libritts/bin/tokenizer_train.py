...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to mls-english_cuts_train_1.jsonl.gz",
    )
    parser.add_argument(
        "--wav-path",
        type=str,
...
def main():
    ...
    i=args.train_part_path.split('_')[-1].split('.')[0]
    cut_set = CutSet.from_file(args.train_part_path)
        ...
        if (
            ...
            for c in tqdm(cut_set): 
                if c.supervisions[0].custom ==None:
                    c.supervisions[0].custom = {}                        #also need to change in this :/lhotse/audio.py 
                if args.wav_path == "":
    ...
    if args.text_extractor:
        ...
        unique_phonemes_file = f"{args.output_dir}/unique_text_tokens_train_{i}.k2symbols"
