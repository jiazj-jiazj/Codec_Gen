...
CUDA_VISIBLE_DEVICES=0 nohup python -u egs/libritts/bin/print_hubert.py --model-name valle --norm-first true --add-prenet ${add_prenet} --decoder-dim 1024 --nhead 16 --num-decoder-layers 12 \
--share-embedding true \
--nums ${num_runs} \
--semantic-tokens /mnt/zhijun/LibriTTS/data/vc_tokenized_v2/unique_semantic_tokens.k2symbols \
--semantic-sys-dir /dev_huaying/zhijun/data/test_vc/test_accent_remove/source \
--audio-prompts-dir /dev_huaying/zhijun/data/test_vc/test_accent_remove/prompt \
--input-semantic True \
--prefix-mode 1 \
...
--top-k ${top_k} --temperature 1.0 \
> egs/libritts/log/hubert_dir_${basestr}.txt 2>&1 &
