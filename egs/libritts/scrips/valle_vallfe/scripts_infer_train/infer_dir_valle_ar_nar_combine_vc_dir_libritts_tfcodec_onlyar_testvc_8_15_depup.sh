...
top_k=10
add_prenet="False"  
CUDA_VISIBLE_DEVICES=1 python egs/libritts/bin/combine_ar_nar_vc_dir_onlyar.py --model-name valle --norm-first true --add-prenet ${add_prenet} --decoder-dim 1024 --nhead 16 --num-decoder-layers 12 \
...
--shared-linear False \
--semantic-depup True \
--checkpoint1 /dev_huaying/zhijun/data/valle-tensorboard-models/vc/only_ar_depup/best-valid-loss-46.pt \
--top-k ${top_k} --temperature 1.0 \
--num-quantizers 16 \
--input-codec 1 \
--outputdir-name "converted_vc_onlyar_test_vc_hubert_depup_test_46"
