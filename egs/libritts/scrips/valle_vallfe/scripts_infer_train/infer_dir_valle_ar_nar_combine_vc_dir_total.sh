...
CUDA_VISIBLE_DEVICES=1 nohup python -u egs/libritts/bin/combine_ar_nar_vc_dir_total.py --model-name valle --norm-first true --add-prenet ${add_prenet} --decoder-dim 1024 --nhead 16 --num-decoder-layers 12 \
--share-embedding true \
--nums ${num_runs} \
--semantic-tokens /scratch/data/Libritts/tokenized/unique_semantic_tokens.k2symbols \
--input-dir /dev_huaying/zhijun/data/test_vc \
--input-semantic True \
--prefix-mode 1 \
...
--top-k ${top_k} --temperature 1.0 \
> egs/libritts/log/vc_dir_total_${basestr}.txt 2>&1 &
