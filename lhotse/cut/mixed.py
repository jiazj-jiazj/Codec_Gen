...
class MixedCut(Cut):
    ...
    @rich_exception_info
    def load_features(self, mixed: bool = True, dir_path=None) -> Optional[np.ndarray]:
        """
        ...
        """
        # print(dir_path)
        # print("/home/v-zhijunjia/valle-4-23/lhotse/cut/mixed.py/load_features")
        ...
        if mixed and all(isinstance(t.cut, PaddingCut) for t in self.tracks[1:]):
            ...
            first_cut_feats = first_cut.load_features(dir_path=dir_path)
        ...
        if first_cut.id != reference_cut.id:
            reference_feats = reference_cut.load_features(dir_path=dir_path)
        ...
        mixer = FeatureMixer(
            ...
            base_feats=first_cut.load_features(dir_path=dir_path),
        ...
        for pos, track in enumerate(self.tracks[1:], start=1):
            if pos == reference_pos and reference_feats is not None:
                ...
                feats = track.cut.load_features(dir_path=dir_path)
            mixer.add_to_mix(
                feats=feats,
                snr=track.snr,
                offset=track.offset,
