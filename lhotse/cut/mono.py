...
class MonoCut(DataCut):
    ...
    @rich_exception_info
    def load_features(self, dir_path) -> Optional[np.ndarray]:
        """
        ...
        # print(f"mono load_features")
        if self.has_features:

            feats = self.features.load(start=self.start, duration=self.duration, dir_path=dir_path)
