...
def prepare_l2_arctic(
    ...
    for part in train_val_test:
        corpus_dir = corpus_dir_bef.joinpath(part)
        print(corpus_dir)
        ...
        if output_dir is not None:
            ...
            for key, manifests in splits.items():
                manifests["recordings"].to_file(
                    output_dir / f"l1_l2_arctic_recordings_{part}.jsonl.gz"
                )
                manifests["supervisions"].to_file(
                    output_dir / f"l1_l2_arctic_supervisions_{part}.jsonl.gz"
                )
        print(f"{part} finished")
...
if __name__=="__main__":
    #need to rewrite
    prepare_l2_arctic("/raid/dataset/l1-l2arctic", "/home/v-zhijunjia/zhijundata_small_v2/data/l1_l2_arctic/lhotse_data_all_accents_native_l1_l2_arctic_v2")