...
def collate_features(
    ...
    if executor is None:
        for idx, cut in enumerate(cuts):
            # print(dir_path)
            # print("/home/v-zhijunjia/valle-4-23/lhotse/dataset/collation.py")
            features[idx] = _read_features(cut, dir_path)
    else:
...
def _read_features(cut: Cut, dir_path) -> torch.Tensor:

    return torch.from_numpy(cut.load_features(dir_path=dir_path))
