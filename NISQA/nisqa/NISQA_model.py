...
class nisqaModel(object):
    ...
    def predict(self):
        ...
        if self.args['output_dir']:
            ...
            # print(f"self.ds_val.df:{self.ds_val.df}")

            # file_names = self.ds_val.df['deg']
            # mos_preds = self.ds_val.df['mos_pred']
            # score_list = {}

            # for file_name, mos_pred in zip(file_names, mos_preds):

            #     prompt_name = file_name.split("prompt1_")[1].split("_sys2")[0]+ ".wav"
            #     if prompt_name not in score_list.keys():
            #         score_list[prompt_name]=[]
            #     score_list[prompt_name].append({file_name:mos_pred})

            # max_score_dict = {}  
            # for prompt, score_dicts in score_list.items():  
            #     # 初始化最大 score 及其对应的 wav 文件  
            #     max_score = -float('inf')  # 或者一个非常小的数  
            #     max_score_wav = None  
            
            #     # 遍历 prompt 下的每一个 score 字典  
            #     for score_dict in score_dicts:  
            #         # 获取 wav 文件和对应的 score  
            #         wav, score = next(iter(score_dict.items()))  
            
            #         # 如果当前 score 大于已知的最大 score，则更新最大 score 和 wav 文件  
            #         if score > max_score:  
            #             max_score = score  
            #             max_score_wav = wav  
            
            #     # 更新 max_score_dict 来存储最大 score 的 wav 文件和对应的 score  
            #     max_score_dict[prompt] = {max_score_wav: max_score}  
            
            # # 打印结果  
            # best_score = []
            # for prompt in max_score_dict:  
            #     best_score.append(max_score_dict[prompt])
            # print(f"best_score:{best_score}")
            # total_best_score=0.0
            # num_best_score=0
            # for xx in best_score:
            #     for key in xx.keys():
            #         total_best_score+=float(xx[key])
            #         num_best_score+=1
            
            # ave_best_score = total_best_score/num_best_score
            # print(f"ave_best_score:{ave_best_score}")

            mos_all = self.ds_val.df['mos_pred']
            for xx in mos_all:
                total_mos+=xx
                num_mos+=1
            ave_mos = total_mos/num_mos
            print(f"mos_num is {num_mos}")
            print(f"average_mos is {ave_mos}")

            self.ds_val.df['average_mos'] = ave_mos

            print(os.path.join(self.args['output_dir'], self.args['file_name']))
            with open(os.path.join(self.args['output_dir'], self.args['file_name']), 'w') as f:
                f.write(self.args['file_name'])
                f.write(str(f"mos_num is {num_mos}"))
                f.write(str(f'average_mos is {ave_mos}'))

            # self.ds_val.df.to_csv(
            #     os.path.join(self.args['output_dir'], self.args['file_name']), 
            #     index=False)
    ...
    def _loadDatasetsFolder(self):
        ...
        if self.args['file_mode'] == 1:
            for root, dirs, files in os.walk(self.args['data_dir']):  
                for file in files:  
                    if file.endswith('.wav') and file.startswith('gt_'):  
                        files1.append(os.path.join(root, file))  
                    elif file.endswith('.flac') and file.startswith('gt_'):  
                        files2.append(os.path.join(root, file))
            data_dir = self.args['data_dir']
        elif self.args['file_mode'] == 0:
            for root, dirs, files in os.walk(self.args['data_dir']):  
                for file in files:  
    ...
    def _loadModel(self):    
        ...
        if self.args['pretrained_model']:
            ...
            self.args['model'] = checkpoint['args']['model']
