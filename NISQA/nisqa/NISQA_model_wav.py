...
class nisqaModel(object):
    ...
    def predict_wav(self, y, sr):
        ...
        if seg_length is not None:
            x_spec_seg, n_wins = NL.segment_specs("xx",
                spec,
                seg_length,
                seg_hop_length,
                max_length)
        ...
        model.to(dev)
        model.eval()
        n_wins = torch.tensor([n_wins.item()], dtype=torch.int64)  
        x_spec_seg = x_spec_seg.unsqueeze(0)  
        
        # 将张量移至 GPU（使用 CUDA）  
        #(6000 1 48 15)
        with torch.no_grad():
            score = model(x_spec_seg.to(dev), n_wins.to(dev)).cpu().numpy()
