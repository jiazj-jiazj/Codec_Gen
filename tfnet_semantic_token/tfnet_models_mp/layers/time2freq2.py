...
class Time2Freq2(nn.Module):
    def __init__(self, frm_size, shift, win_len, hop_len, n_fft, config=None, enableBP=False, power=None):
    ...
    def forward(self, input_1d, oneframe = False):
        ...
        if self.use_compressed_input:
            if self.enableBP or self.config["use_learnable_compression"]:
        ...
        if self.config is not None and self.config["use_online_feature_norm"]:
