...
def audioread_resample(path, norm=False, start=0, stop=None, target_level=-25, target_sample_rate=16000):  
    '''Function to read audio'''  
...
def normByActive(input_audio, target_level=-25, fs=16000):
    rms, _ = active_rms(clean=input_audio, noise=input_audio, fs=fs)
    scalar = 10 ** (target_level / 20) / (rms+EPS)
    input_audio = input_audio * scalar
    return input_audio
...
def segmental_snr_mixer(params, clean, noise, snr, fs=16000, target_level=-25, clipping_threshold=0.99):
    ...
    clean = clean/(max(abs(clean))+EPS)
    noise = noise/(max(abs(noise))+EPS)
    rmsclean, rmsnoise = active_rms(clean=clean, noise=noise, fs=fs)
...
def normalize_segmental_rms_dereverb(audio, audiodereverb, rms, target_level=-25):
    '''Normalize the signal to the target level
    based on segmental RMS'''
    scalar = 10 ** (target_level / 20) / (rms+EPS)
    audio = audio * scalar
    audiodereverb = audiodereverb * scalar
    return audio, audiodereverb
    
# def segmental_snr_mixer_dereverb(params, cleandereverb, clean, noise, snr, fs=16000, target_level=-25, clipping_threshold=0.99):
    # '''Function to mix clean speech and noise at various segmental SNR levels'''
    # cfg = params['cfg']
    # if len(clean) > len(noise):
        # noise = np.append(noise, np.zeros(len(clean)-len(noise)))
    # else:
        # clean = np.append(clean, np.zeros(len(noise)-len(clean)))
        # cleandereverb = np.append(cleandereverb, np.zeros(len(noise)-len(cleandereverb)))
    # clean = clean/(max(abs(clean))+EPS)
    # noise = noise/(max(abs(noise))+EPS)
    # cleandereverb = cleandereverb/(max(abs(clean))+EPS)
    # rmsclean, rmsnoise = active_rms(clean=clean, noise=noise, fs=fs)
    # clean, cleandereverb = normalize_segmental_rms_dereverb(clean, cleandereverb, rms=rmsclean, target_level=target_level)
    # noise = normalize_segmental_rms(noise, rms=rmsnoise, target_level=target_level)
    # # Set the noise level for a given SNR
    # #noisescalar = rmsclean / (10**(snr/20)) / (rmsnoise+EPS)
    # noisescalar = 1.0 / (10**(snr/20))
    ...
    # noisyspeech = noisyspeech * scalarnoisy
    # clean = clean * scalarnoisy
    # cleandereverb = cleandereverb * scalarnoisy
    # noisenewlevel = noisenewlevel * scalarnoisy
    # # Final check to see if there are any amplitudes exceeding +/- 1. If so, normalize all the signals accordingly
    # if is_clipped(noisyspeech):
        # noisyspeech_maxamplevel = max(abs(noisyspeech))/(clipping_threshold-EPS)
        # noisyspeech = noisyspeech/noisyspeech_maxamplevel
        # clean = clean/noisyspeech_maxamplevel
        # cleandereverb = cleandereverb/noisyspeech_maxamplevel
        # noisenewlevel = noisenewlevel/noisyspeech_maxamplevel
        # noisy_rms_level = int(20*np.log10(scalarnoisy/noisyspeech_maxamplevel*(rmsnoisy+EPS)))

    # return cleandereverb, clean, noisenewlevel, noisyspeech, noisy_rms_level
...
def unpairsegmental_snr_mixer(params, clean, noise, snr, fs=16000, target_level=-25, clipping_threshold=0.99):
    '''Function to mix clean speech and noise at various segmental SNR levels'''
    cfg = params['cfg']
    if len(clean) > len(noise):
        noise = np.append(noise, np.zeros(len(clean)-len(noise)))
    ...
    clean = clean/(max(abs(clean))+EPS)
    noise = noise/(max(abs(noise))+EPS)
    rmsclean, rmsnoise = active_rms_unpair(clean=clean, noise=noise, fs=fs)
    clean = normalize_segmental_rms(clean, rms=rmsclean, target_level=target_level)
    noise = normalize_segmental_rms(noise, rms=rmsnoise, target_level=target_level)
    # Set the noise level for a given SNR
    #noisescalar = rmsclean / (10**(snr/20)) / (rmsnoise+EPS)
...
def unpairsegmental_snr_mixer_dereverb(params, speech, noise, targetspeech, snr, fs=16000, target_level=-25, clipping_threshold=0.99):
    '''Function to mix clean speech and noise at various segmental SNR levels'''
    cfg = params['cfg']
    if len(speech) > len(noise):
        noise = np.append(noise, np.zeros(len(speech)-len(noise)))
    else:
        speech = np.append(speech, np.zeros(len(noise)-len(speech)))
        targetspeech = np.append(targetspeech, np.zeros(len(noise)-len(targetspeech)))
    max_speech = max(abs(speech))
    speech = speech/(max_speech+EPS)
    noise = noise/(max(abs(noise))+EPS)
    targetspeech = targetspeech/(max_speech+EPS)
    rmsclean, rmsnoise = active_rms_unpair(clean=speech, noise=noise, fs=fs)
    speech, targetspeech = normalize_segmental_rms_dereverb(speech, targetspeech, rms=rmsclean, target_level=target_level)
    noise = normalize_segmental_rms(noise, rms=rmsnoise, target_level=target_level)
    # Set the noise level for a given SNR
    #noisescalar = rmsclean / (10**(snr/20)) / (rmsnoise+EPS)
    noisescalar = 1.0 / (10**(snr/20))
    ...
    # Mix noise and clean speech
    noisyspeech = speech + noisenewlevel
    # Randomly select RMS value between -15 dBFS and -35 dBFS and normalize noisyspeech with that value
    # There is a chance of clipping that might happen with very less probability, which is not a major issue. 
    noisy_rms_level = np.random.randint(params['target_level_lower'], params['target_level_upper'])
    rmsnoisy = (noisyspeech**2).mean()**0.5
    scalarnoisy = 10 ** (noisy_rms_level / 20) / (rmsnoisy+EPS)
    noisyspeech = noisyspeech * scalarnoisy
    targetspeech = targetspeech * scalarnoisy
    speech = speech * scalarnoisy
    noisenewlevel = noisenewlevel * scalarnoisy
    # Final check to see if there are any amplitudes exceeding +/- 1. If so, normalize all the signals accordingly
    if is_clipped(noisyspeech):
        noisyspeech_maxamplevel = max(abs(noisyspeech))/(clipping_threshold-EPS)
        noisyspeech = noisyspeech/noisyspeech_maxamplevel
        targetspeech = targetspeech/noisyspeech_maxamplevel
        noisenewlevel = noisenewlevel/noisyspeech_maxamplevel
        speech = speech/noisyspeech_maxamplevel
        noisy_rms_level = int(20*np.log10(scalarnoisy/noisyspeech_maxamplevel*(rmsnoisy+EPS)))

    return targetspeech, noisenewlevel, noisyspeech, noisy_rms_level
    
def active_rms_unpair(clean, noise, fs=16000, energy_thresh=-50):
    '''Returns the clean and noise RMS of the noise calculated only in the active portions'''
    window_size = 100 # in ms
    window_samples = int(fs*window_size/1000)
    sample_start = 0
...
def preSilenceTrim(audio, fs=16000, energy_thresh=0.13, target_level=-25):
    '''Return the percentage of the time the audio signal is above an energy threshold'''

    audio_norm = normalize(audio, target_level)
    window_size = 50 # in ms
    window_samples = int(fs*window_size/1000)
    sample_start = 0
    cnt = 0
    ...
    while sample_start < len(audio_norm):
        sample_end = min(sample_start + window_samples, len(audio_norm))
        audio_win = audio_norm[sample_start:sample_end]
        frame_rms = 20*np.log10(sum(audio_win**2)+EPS)
        frame_energy_prob = 1./(1+np.exp(-(a+b*frame_rms)))
    ...
    if sample_start <= window_samples:
        audio_trimmed = audio
    else:
        audio_trimmed = audio[sample_start-window_samples:]
    return audio_trimmed
    
def postSilenceTrim(audio, fs=16000, energy_thresh=0.13, target_level=-25):
    '''Return the percentage of the time the audio signal is above an energy threshold'''
    
    audio_inv = np.flip(audio, 0)
    audio_inv = preSilenceTrim(audio_inv, fs, energy_thresh, target_level)    
    return np.flip(audio_inv, 0)
