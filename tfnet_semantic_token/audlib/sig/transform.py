...
--------
stproc, fbanks
TODO: comment out or delete the ununsed function
...
from .spectral import logmag, logpow, realcep, compcep
...
def stft(sig, sr, wind, hop, nfft, synth=False):
...
def istft(sframes, sr, wind, hop, nfft):
...
def stpowspec(sig, sr, wind, hop, nfft, synth=False):
    ...
    spec = stft(sig, sr, wind, hop, nfft, synth=synth)
...
def stmfcc(sig, sr, wind, hop, nfft, melbank, synth=False):
    ...
    return dct(
        np.log(stmelspec(sig, sr, wind, hop, nfft, melbank, synth=synth)), norm="ortho"
    )


def stlogm(sig, sr, wind, hop, nfft, synth=False, floor=-10.0):
    ...
    """
    return logmag(
        stft(sig, sr, wind, hop, nfft, synth=synth), floor=floor
    )


def stlogpow(sig, sr, wind, hop, nfft, synth=False, floor=-30.0):
    """Short-time log power spectra.

    See Also
    --------
    spectral.logpow, stft
    """
    return logpow(
        stft(sig, sr, wind, hop, nfft, synth=synth), floor=floor
    )
...
def strcep(sig, sr, wind, hop, n, synth=False, floor=-10.0):
...
def stccep(sig, sr, wind, hop, n, synth=False, floor=-10.0):
