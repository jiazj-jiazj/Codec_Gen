...
from valle.data.dataset import SpeechSynthesisDataset, SpeechSynthesisDataset_infer
...
class TtsDataModule:
    ...
    def train_dataloaders(
        ...
        print(f"Real num_workers:{self.args.num_workers}")
    ...
    def train_cuts(self) -> CutSet:
        ...
        print(f"load_train_file : {self.args.manifest_dir}, {self.args.train_dir_name}")
        return load_manifest_lazy(
            self.args.manifest_dir / self.args.train_dir_name
        )

    @lru_cache()
    def train_cuts_sec(self) -> CutSet:
        logging.info("About to get train cuts")
        print(f"load_train_file_sec :{self.args.manifest_dir_sec} {self.args.train_dir_name_sec}")
        return load_manifest_lazy(
            self.args.manifest_dir_sec / self.args.train_dir_name_sec
        )
    ...
    def dev_cuts(self) -> CutSet:
        ...
        print(f"load_val_file : {self.args.manifest_dir}, {self.args.val_dir_name}")
        return load_manifest_lazy(self.args.manifest_dir / self.args.val_dir_name)

    @lru_cache()
    def dev_cuts_sec(self) -> CutSet:
        logging.info("About to get dev cuts")
        print(f"load_val_file_sec :{self.args.manifest_dir_sec} {self.args.val_dir_name_sec}")
        return load_manifest_lazy(self.args.manifest_dir_sec / self.args.val_dir_name_sec)
...
class TtsDataModule_infer:
    ...
    def train_dataloaders(
        ...
        if self.args.on_the_fly_feats:
            ...
            # transforms = [PerturbSpeed(factors=[0.9, 1.1], p=2/3)] + transforms   # noqa
            # Drop feats to be on the safe side.
            train = SpeechSynthesisDataset_infer(
                get_text_token_collater(self.args.text_tokens),
                get_text_token_collater(self.args.semantic_tokens),
                cut_transforms=transforms,
                feature_input_strategy=OnTheFlyFeatures(get_fbank_extractor()),
