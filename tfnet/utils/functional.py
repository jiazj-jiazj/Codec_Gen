...
# fs
def formant_shift(sound: parselmouth.Sound,set_formant=None) -> parselmouth.Sound:
    ...
    if set_formant is not None:
        formant_shifting_ratio = set_formant
...
# fs
def pitch_shift(sound: parselmouth.Sound,set_pitch_shift_ratio=None,set_pitch_range_ratio=None) -> parselmouth.Sound:
    """designed for formant shifting(fs) in the paper
    Args:
        sound: parselmouth Sound object
    Returns:
    ...
    if set_pitch_shift_ratio is not None:
        pitch_shift_ratio = set_pitch_shift_ratio
    if set_pitch_range_ratio is not None:
        pitch_range_ratio = set_pitch_range_ratio
    sound_new = apply_formant_and_pitch_shift(
        sound,
...
# fs & pr
def formant_and_pitch_shift(sound: parselmouth.Sound,do_formant=False,do_pitch=False) -> parselmouth.Sound:
    ...
    if do_formant:
        ...
        formant_shifting_ratio = PRAAT_CHANGEGENDER_FORMANTSHIFTRATIO_DEFAULT
...
def transform(wav: torch.Tensor, sr: int, do_peq=False,do_pitch=False,do_formant=False):
    if do_peq:
        wav = peq(wav, sr)
    wav_numpy = wav.numpy()
    sound = wav_to_Sound(wav_numpy, sampling_frequency=sr)
    sound = formant_and_pitch_shift(sound,do_formant=do_formant,do_pitch=do_pitch)
    wav = torch.from_numpy(sound.values).float().squeeze(0)
    return wav

# def transform(wav: torch.Tensor, sr: int, do_peq=False,do_pitch=False,do_formant=False,pitch_ratio=None,pitch_range_ratio=None,formant=None):
#     if do_peq:
#         wav = peq(wav, sr)
#     wav_numpy = wav.numpy()
#     sound = wav_to_Sound(wav_numpy, sampling_frequency=sr)
#     if do_pitch and do_formant:
#         sound = formant_and_pitch_shift(sound)
#     else:
#         if do_pitch:
#             sound = pitch_shift(sound, pitch_ratio,pitch_range_ratio)
#         if do_formant:
#             sound = formant_shift(sound,formant)
#     wav = torch.from_numpy(sound.values).float().squeeze(0)
#     return wav

# normalize frame number
# return updated start sample idx and end sample idx
def frame_num_normalize(total_frames, max_frame_num, frame_hop, frame_size):
    max_start_frame = total_frames - max_frame_num
    start_frame = torch.randint(0, max_start_frame, (1, ))
    start = start_frame * frame_hop
    end = start + (max_frame_num - 1) * frame_hop + frame_size
    return start, end

# normalize segment length
# return updated starting sample idx and ending sample idx
def segment_len_normalize(total_segment_samples, max_segment_samples, split='train'):
    max_start_idx = total_segment_samples - max_segment_samples
    start_idx = torch.randint(0, max_start_idx, (1, )) if split == 'train' else 0
    end_idx = start_idx + max_segment_samples
    return start_idx, end_idx

# normalize feature number
# return updated starting feature id and ending feature id
def feat_num_normalize(feat_num, max_feat_num):
    max_start_idx = feat_num - max_feat_num
    start_idx = torch.randint(0, max_start_idx, (1, ))
    end_idx = start_idx + max_feat_num
    return start_idx, end_idx
...
if __name__ == "__main__":
    ...
    wav_path = r'test.wav'
    out_dir = r'augmentation'
    wav_path_out1 = r'D:\code\codec\inference_debug_case\augmentation\test_aug.wav'
    wav_path_out2 = r'D:\code\codec\inference_debug_case\augmentation\test_pitch.wav'
    wav_path_out3 = r'D:\code\codec\inference_debug_case\augmentation\test_formant.wav'
    wav,_ = audioread(wav_path,sr=16000)
    wav = wav_to_Tensor(wav)
    wav_out2 = transform(wav,16000,do_pitch=False,do_formant=True)
    audiowrite(wav_out2.cpu().numpy(),sr=16000,outpath=wav_path_out1)
