...
class GumbelVectorQuantizer(nn.Module):
    ...
    def dequantize_from_logits(self, logits):
        ## logits [B,T,N] N:codebook size
        bsz, tsz = logits.shape[0], logits.shape[1]
        logits = logits.view(bsz * tsz, -1)
        vars = self.embedding  # [1, M, D]
        if self.combine_groups:
            vars = vars.repeat(1, self.groups, 1)

        logits = logits.unsqueeze(-1) * vars  # [BT, M, D]
        logits = logits.view(bsz * tsz, self.groups, self.n_embeddings, -1)
        logits = logits.sum(-2)  # [BT, D]
        quantized = logits.view(bsz, tsz, -1) # [B, T, D]
        return quantized

    def get_extra_losses_type2(self, target_entropy):
        mae_loss = nn.L1Loss().to(self.code_entropy)
        target_entropy = target_entropy.clone().detach()
