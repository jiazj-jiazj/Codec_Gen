...
class GumbelVectorQuantizer_Predictive(nn.Module):
    def __init__(self, config, input_dim, n_embeddings, groups, combine_groups=False):
        ...
        if config["fuse_type"] == 'conv':
            self.fuse_conv1_list = nn.ModuleList(nn.Conv1d(2*input_dim_per_group, embedding_dim, kernel_size=1, stride=1, bias=False) for i in range(groups))
            self.nonlinearity1_list = nn.ModuleList(nn.PReLU(init=0.5) for i in range(groups)) 
            self.fuse_conv2_list = nn.ModuleList(nn.Conv1d(input_dim_per_group+embedding_dim, input_dim_per_group, kernel_size=1, stride=1, bias=False) for i in range(groups))
    ...
    def predictor(self, x, state=None): # (B,C,T)
        ...
        if self.config["prediction_type"] == 'conv':
            ...
            ctx_in_list = torch.chunk(x, self.groups, dim=1)
            for ii in range(self.groups):
                ctx_out_1 = self.context_nonlinearity1_list[ii](self.context_gen_list_1[ii](ctx_in_list[ii]))            
    ...
    def forward(self, x, target_entropy=0, fuzz_entropy=0, ar_signal=None, epo=None):
        ...
        if self.training and (ar_signal is not None):
            ...
            if self.config["fuse_type"] == 'conv':
                ...
                cur_x_list = []
                for ii in range(self.groups):            
                    merge_out = self.nonlinearity1_list[ii](self.fuse_conv1_list[ii](torch.cat((x_list[ii], ctx_out_list[ii]), dim=1)))
            ...
            if self.config["fuse_type"] == 'conv':
                ...
                for ii in range(self.groups):
                    merge_out_list.append(self.nonlinearity2_list[ii](self.fuse_conv2_list[ii](torch.cat((output_q_list[ii], ctx_out_list[ii]), dim=1))))
            ...
            for ii in range(tsz):
                if self.config["fuse_type"] == 'conv':
                    ...
                    cur_x_list = []
                    for gg in range(self.groups):            
                        merge_out = self.nonlinearity1_list[gg](self.fuse_conv1_list[gg](torch.cat((x_list[gg], context_list[gg]), dim=1)))
                        cur_x_list.append(merge_out)
