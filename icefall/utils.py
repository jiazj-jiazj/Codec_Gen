...
class MetricsTracker(collections.defaultdict):
    ...
    def __str__(self) -> str:
        ...
        num_frames_split = [self.get(f"frames_{i}", 0) for i in range(1, 8)] 
        for i, num_frames in enumerate(num_frames_split):
            if num_frames==0:
                continue
            ans_frames += "over " + str(num_frames) + f" frames_{i+1} "
    ...
    def update_with_all_metrics(self, other: "MetricsTracker", reset_interval: int) -> None:  
        for metric_name in other.keys():  
            self[metric_name] = (  
                self[metric_name] * (1 - 1 / reset_interval)  
            ) + other[metric_name] * (1 / reset_interval)  
    ...
    def norm_items(self) -> List[Tuple[str, float]]:
        ...
        num_frames_split = [self.get(f"frames_{i}", 1) for i in range(1, 16)]  
        ...
        for k, v in self.items():
            ...
            if k.startswith("frames_"):  
                continue  
            if k.startswith("NarTop10Accuracy_"):  
                try:  
                    index = int(k.split("_")[-1])  
                    # if 1 <= index <= 7:  
                    frame_index = index - 1  
                    norm_value = float(v) / num_frames_split[frame_index]
                    # else: 
